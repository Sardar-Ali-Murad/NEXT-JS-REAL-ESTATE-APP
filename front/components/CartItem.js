...
const CartItem = ({all}) => {
   ...
   function minus(){
    ...
    React.useEffect(()=>{
      const start=async ()=>{
         let {data}=await axios.patch(`http://localhost:5000/api/v1/Orders/${all._id}`,{quantity:count},  {withCredentials:true})
      }
       start()
      UOrders()
    },[count])
    ...
    const del=async ()=>{
     let {data}=await axios.delete(`http://localhost:5000/api/v1/Orders/${all?._id}`,{withCredentials:true})
     UOrders()
    }
